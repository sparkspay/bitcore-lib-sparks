 ...
 * @property {boolean} isValid
 */
function SimplifiedGNListEntry(arg, network) {
  if (arg) {
    if (arg instanceof SimplifiedGNListEntry) {
      return arg.copy();
    } else if (BufferUtil.isBuffer(arg)) {
      return SimplifiedGNListEntry.fromBuffer(arg, network);
    } else if (_.isObject(arg)) {
      return SimplifiedGNListEntry.fromObject(arg);
    } else if (arg instanceof SimplifiedGNListEntry) {
      return arg.copy();
    } else if (isHexString(arg)) {
      return SimplifiedGNListEntry.fromHexString(arg, network);
    } else {
      throw new TypeError('Unrecognized argument for SimplifiedGNListEntry');
    }
  }
}

/**
 * Parse buffer and returns SimplifiedGNListEntry
 * @param {Buffer} buffer
 * @param {string} [network]
 * @return {SimplifiedGNListEntry}
 */
SimplifiedGNListEntry.fromBuffer = function fromBuffer(buffer, network) {
  var bufferReader = new BufferReader(buffer);

  return SimplifiedGNListEntry.fromObject({
 ...
 * @return {SimplifiedGNListEntry}
 */
SimplifiedGNListEntry.fromHexString = function fromString(string, network) {
  return SimplifiedGNListEntry.fromBuffer(Buffer.from(string, 'hex'), network);
};
 ...
 * @return {Buffer}
 */
SimplifiedGNListEntry.prototype.toBuffer = function toBuffer() {
  this.validate();
 ...
 * @return {SimplifiedGNListEntry}
 */
SimplifiedGNListEntry.fromObject = function fromObject(obj) {
  var SMLEntry = new SimplifiedGNListEntry();
...
SimplifiedGNListEntry.prototype.validate = function validate() {
...
SimplifiedGNListEntry.prototype.toObject = function toObject() {
  return {
 ...
 * @return {Buffer}
 */
SimplifiedGNListEntry.prototype.calculateHash = function calculateHash() {
  return Hash.sha256sha256(this.toBuffer());
};

/**
 * Creates a copy of SimplifiedGNListEntry
 * @return {SimplifiedGNListEntry}
 */
SimplifiedGNListEntry.prototype.copy = function copy() {
  return SimplifiedGNListEntry.fromBuffer(this.toBuffer(), this.network);
};

module.exports = SimplifiedGNListEntry;
